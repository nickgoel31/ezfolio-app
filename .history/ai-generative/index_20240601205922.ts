...
export const googleGeminiGenerativeAi = async (questionTitle:string, questionDesc:string) => {
    ...
    const model = genAI.getGenerativeModel({
        ...
        Avoid using <h1> tags instead use h3 or h4, after using heading tag make sure to leave a blank <p></p> to leave a line for formatting
        make sure to use heading tags only for headings and not for paragraphs or normal information
        Make sure you answer to the prompt given by user and nothing else`,
        generationConfig
    })
