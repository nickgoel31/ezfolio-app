...
async function findUsersByExperience (experiences:Experience[]){
    const users = await db.userPage.findMany()
    const similarUsers:string[] = []
    ...
    for (const user of users){
        ...
        const similarityScore = commonExperience.length > 0 ? commonExperience.length / experiences.length : 0
