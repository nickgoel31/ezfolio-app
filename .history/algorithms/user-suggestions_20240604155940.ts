...
export const userSuggestionsAlgorithm = async () => {
    ...
    const usersWithSimilarSkillsArray: UserPage[] = [];
    const usersWithSimilarExperienceArray: UserPage[] = [];

    // Find users with similar skills
    const usersWithSimilarSkills:UserPage[] = await findUsersBySkills(currentUserSkills);
    usersWithSimilarSkillsArray.push(...usersWithSimilarSkills);

    const usersWithSimilarExperience = await findUsersByExperience(experiences);
    usersWithSimilarExperienceArray.push(...usersWithSimilarExperience);
    ...
    const combinedUsers: UserPage[] = [...usersWithSimilarSkills, ...usersWithSimilarExperience];
    const uniqueUsers = combinedUsers.filter(
        (user) => !combinedUsers.some((dup) => dup.id === user.id && dup !== user)
    );
    const uniqueUsersMinusCurrentUser = uniqueUsers.filter((user) => user.username !== currentUser.username)
    console.log(uniqueUsers)

    return uniqueUsersMinusCurrentUser;
}
