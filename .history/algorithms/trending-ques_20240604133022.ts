...
export const trendingQuestionsAlgorithm = async () => {
    ...
    const topQuestions:QuestionPost = [];
    for (const [postId, likes] of sortedLikes) {
        const post = await db.questionPost.findUnique({
            where: {
                id: postId
            }
        });
        if (post) {
            topQuestions.push(post); // Assuming the question field exists in the post object
        }
    }
