...
export const userSuggestionsAlgorithm = async () => {
    ...
    const uniqueUsers = Array.from(new Set(similarUsers)).filter(id => id !== currentUser.id);

    return uniqueUsers;
}


async function findUsersBySkills (currentUserSkills:UserPage[]){
    ...
    for (const user of users){
        ...
        const commonSkills = userSkills.filter(skill => currentUserSkills.skills.includes(skill))
