...
import { getAdvancedPortfolioThemeColorOfAPTheme, getAdvancedPortfolioThemeOfUserPage, getAllAdvancedPortfolioThemes, getCompatibleThemeColorsWithThemeLayouts } from '@/actions/adv-port/get-adv-port';
...
const AppearancePage = async ({params}:{params:{userId:string}}) => {
  ...
  if(!userPage) {
    ...
    const currentAdvancedPortfolioThemeColor = await getAdvancedPortfolioThemeColorOfAPTheme(currentAdvancedPortfolioTheme.advPortThemeColorId)
    if(!currentAdvancedPortfolioThemeColor) return  <div>Error while fetching data</div>

    const compatibleThemeColorsWithCurrentThemeLayout = await getCompatibleThemeColorsWithThemeLayouts(currentAdvancedPortfolioTheme.id)
    if(!compatibleThemeColorsWithCurrentThemeLayout) return  <div>Error while fetching data</div>
    ...
    <div className=''>
        <div className='flex gap-4'>
          <div className='flex-[2] space-y-5'>
            ...
            <ModernPortfolioToggle userPageId={userPage.id} advancedPortfolioThemes={advancedPortfolioThemes} isAdvancedPortfolioToggled={isAdvancedPortfolioToggled} currentAdvancedPortfolioTheme={currentAdvancedPortfolioTheme} currentAdvancedPortfolioThemeColor={currentAdvancedPortfolioThemeColor} tagline={userPage.tagline} compatibleThemeColorsWithCurrentThemeLayout={compatibleThemeColorsWithCurrentThemeLayout}/>
