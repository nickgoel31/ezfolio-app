...
const AppearancePage = async ({params}:{params:{userId:string}}) => {
  ...
  if(!userPage) {
    ...
    const customUserpageTheme = await getCustomUserpageTheme()
    if(!customUserpageTheme) return  <div>Error while fetching data</div>
    ...
    const isCustomThemeEnabled = userPage.isCustomUserPageThemeEnabled
