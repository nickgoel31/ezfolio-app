...
const ModernPortfolioToggle = ({userPageId,isAdvancedPortfolioToggled,advancedPortfolioThemes,currentAdvancedPortfolioTheme}:Props) => {
    ...
    const [currentAdvancedPortfolioThemeState, setIsAdvancedPortfolioToggledState] = useState<number>(currentAdvancedPortfolioTheme.id)
    ...
    <div className='border shadow-sm rounded-xl p-5 bg-background w-full space-y-2'>
        ...
        <div className='flex flex-row flex-wrap w-full gap-4'>
            {advancedPortfolioThemes.map((theme) => (
                <div onClick={() => setIsAdvancedPortfolioToggledState(theme.id)} key={theme.id} className={cn('rounded-xl flex flex-col gap-2 items-center justify-center text-neutral-100 px-3 p-3 text-center cursor-pointer border',currentAdvancedPortfolioThemeState === theme.id && "outline outline-offset-2 outline-indigo-700")}>
                    {/* <Image src={`/layout${layout.id}-dark.png`} alt='layoutimg' width={1000} height={1000} className='w-full h-14'/> */}
                    <h6 className='font-medium'>{theme.layoutName}</h6>
                </div>
            ))}
        </div>
    </div>
