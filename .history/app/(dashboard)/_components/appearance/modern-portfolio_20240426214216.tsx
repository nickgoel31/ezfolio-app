    ...
    <div className='border shadow-sm rounded-xl p-5 bg-background w-full space-y-2'>
        ...
        <div className='flex flex-row flex-wrap w-full gap-4'>
            {advancedPortfolioThemes.map((theme) => (
                <div onClick={() => setIsAdvancedPortfolioToggledState(theme.id)} key={layout.id} className={cn('rounded-xl flex flex-col gap-2 items-center justify-center text-neutral-100 px-3 p-3 text-center cursor-pointer border',pageLayoutState === layout.id && "outline outline-offset-2 outline-indigo-700")}>
