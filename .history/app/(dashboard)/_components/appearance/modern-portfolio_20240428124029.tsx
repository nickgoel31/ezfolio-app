                ...
                <div className='space-y-3'>
                    ...
                    <div className='flex flex-row flex-wrap w-full gap-4'>
                        {advancedPortfolioThemes.map((theme) => (
                            <div onClick={() => setIsAdvancedPortfolioThemeState(theme.id)} key={theme.id} className={cn('w-56  h-40 rounded-xl flex flex-col gap-2 items-center justify-center text-neutral-100 p-1 relative text-center cursor-pointer border',currentAdvancedPortfolioThemeState === theme.id && "outline outline-offset-2 outline-indigo-700")} >
                                {theme.thumbnailUrl && (
                                    <Image src={theme.thumbnailUrl} width={1000} height={1000} className='rounded-xl object-w-full h-full' alt={theme.themeName} />
                                )}
