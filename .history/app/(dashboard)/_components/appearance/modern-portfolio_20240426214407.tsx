...
const ModernPortfolioToggle = ({userPageId,isAdvancedPortfolioToggled,advancedPortfolioThemes,currentAdvancedPortfolioTheme}:Props) => {
  ...
  return (
    <div className='border shadow-sm rounded-xl p-5 bg-background w-full space-y-2'>
        ...
        <div className='flex flex-row flex-col flex-wrap w-full gap-4'>
            <div>
                <h4 className='font-medium'>Tagline</h4>

            </div>
            {advancedPortfolioThemes.map((theme) => (
                <div onClick={() => setIsAdvancedPortfolioThemeState(theme.id)} key={theme.id} className={cn('rounded-xl flex flex-col gap-2 items-center justify-center text-neutral-100 px-3 p-3 text-center cursor-pointer border',currentAdvancedPortfolioThemeState === theme.id && "outline outline-offset-2 outline-indigo-700")}>
