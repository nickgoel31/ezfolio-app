  ...
  return (
    <div className='border shadow-sm rounded-xl p-5 bg-background w-full space-y-2'>
            ...
            <div className='flex flex-row flex-wrap w-full gap-4'>
                {layouts.map((scheme) => (
                    <div onClick={() => setColorSchemeState(scheme.id)} key={scheme.id} className={cn('w-28  h-28 rounded-xl flex items-center justify-center text-neutral-100 px-3 p-3 text-center cursor-pointer border',pageLayoutState === scheme.id && "outline outline-offset-2 outline-indigo-700")}>
