  ...
  return (
    <div className='border shadow-sm rounded-xl p-5 bg-background w-full space-y-2'>
            ...
            <div className='flex flex-row flex-wrap w-full gap-4'>
                {layouts.map((layout) => (
                    <div onClick={() => setPageLayoutState(layout.id)} key={layout.id} className={cn('w-28  h-28 rounded-xl flex items-center justify-center text-neutral-100 px-3 p-3 text-center cursor-pointer border',pageLayoutState === layout.id && "outline outline-offset-2 outline-indigo-700")}>
                        <Image src={"/layout1-dark.png"} alt='layout1' width={1000} height={1000} className='w-full h-14'/>
