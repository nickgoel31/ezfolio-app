    ...
    <>
      {!nav.preview ? (
        ...
        <Link href={`/${username}`} target='_blank' className='relative rounded p-2 hover:bg-foreground/10 transition flex gap-3 items-center w-full justify-center lg:justify-start text-sm lg:text-base'>
          <Icon size={18}/>
            <p className='absolute left-0 translate-x-[calc(5vw)] font-medium text-sm p-1 px-2 rounded-md bg-violet-950/10 border items-center gap-2 text-violet-200 group-hover:flex hidden'>{nav.label}
