...
export const UpdatedNavItems = ({nav, userId,username}:{nav:NavItemType, userId:string,username:string}) => {
  ...
  return(
    <>
      {!nav.preview ? (
        <>
          <Link href={`${nav.pathname}/${userId}${nav.href}`} className='rounded p-2 hover:bg-foreground/10 transition flex gap-2 w-full group relative'>
            <Icon size={18}/>
            <p className='group-hover:block hidden absolute translate-x-[100px]'>{nav.label}</p>
          </Link>
