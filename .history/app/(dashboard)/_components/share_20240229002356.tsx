    ...
    <div className={cn("share-div", isOpen ? "min-w-[26vw] p-3 px-4 flex flex-col gap-2" : "p-2 px-3 min-w-0 ")}>
        ...
        {isOpen && isOthersTab && (
            <div className='rounded p-2 px-3 grid grid-cols-3 items-center justify-between gap-4 w-full '>
                ...
                <Link href={`https://twitter.com/intent/tweet?url=${shareLink}&text=${shareText}`} target='_blank'>
                    <FaTwitter size={20}/>
                    <p className='font-medium text-xs'>Whatsapp</p>
                </Link>
                ...
                <Link href={`mailto:?subject=Check out this website!&amp;body=Check out this website`} target='_blank'>
                    <Mail size={20}/>
                    <p className='font-medium text-xs'>Whatsapp</p>
                </Link>
