...
export const UpdatedNavItems = ({nav, userId,username}:{nav:NavItemType, userId:string,username:string}) => {
  ...
  return(
    <>
      {!nav.preview ? (
        <>
          <Link href={`${nav.pathname}/${userId}${nav.href}`} className='rounded p-2 hover:bg-foreground/10 transition flex gap-2 items-center w-full justify-center peer/id'>
            <Icon size={18}/>
            
          </Link>
          <p className='peer/id-hover:block hidden absolute'>{nav.label}</p>
        </>
      ):(
