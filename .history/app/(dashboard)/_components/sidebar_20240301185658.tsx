...
export const UpdatedSidebar = ({userId,username}:{userId:string,username:string}) => {
    ...
    return (
        <div className='hidden lg:block  w-full'>
            ...
            className={cn('updated-sidebar cursor-pointer rounded-xl bg-gradient-to-t from-violet-950/10 to-background border p-5 font-semibold text-lg z-[101] fixed top-8 left-12 transition h-[80px] flex flex-col items-center justify-start ', sidebarOpen && 'h-80')}>
                <span className='absolute bottom-3 w-2 h-2 bg-white rounded-full left-1/2 -translate-x-1/2'/>
                <div className=' h-full flex flex-col justify-between'>
                    <div className='space-y-4'>
                        <Logo /> 

                        {(
                            <div className={cn('space-y-1 transition-opacity overflow-hidden', animation ? "opacity-100 animate-fade-in delay-200 repeat-1" : "animate-fade-out repeat-1 opacity-0")}>
                                {navigationItems.map((nav,index) => (
                                <div className='w-full relative group'>
                                    <UpdatedNavItems key={index} nav={nav} userId={userId} username={username}/>
                                    <p className='hidden group-hover:block absolute text-sm font-medium top-1/2 -translate-y-1/2 left-[150%] border border-foreground/5 rounded p-2 px-4 bg-gradient-to-r from-violet-950/10 to-background'>{nav.label}</p>
                                </div>
                                ))}
                            </div>
                        )}
