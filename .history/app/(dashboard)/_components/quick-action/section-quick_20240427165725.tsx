            ...
            {projects.slice(0,3).map((project) => (
                <div key={project.id} className='border shadow-sm rounded-xl space-y-1 p-4 bg-background'>
                    ...
                    <div className='flex justify-between'>
                        <a href={`/d/${project.id}/edit`} className='text-sm font-medium text-blue-500'>Edit</a>
                        <a href={`/d/${project.id}/delete`} className='text-sm font-medium text-red-500'>Delete</a>
                </div> 
            ))}
