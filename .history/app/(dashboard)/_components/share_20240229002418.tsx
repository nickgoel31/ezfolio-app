    ...
    <div className={cn("share-div", isOpen ? "min-w-[26vw] p-3 px-4 flex flex-col gap-2" : "p-2 px-3 min-w-0 ")}>
        ...
        {isOpen && isOthersTab && (
            <div className='rounded p-2 px-3 grid grid-cols-3 items-center justify-between gap-4 w-full '>
                ...
                <Link href={`whatsapp://send?text=${shareText}%20${shareLink}`} data-action="share/whatsapp/share" target='_blank' className='hover:bg-foreground/5 rounded transition p-2 flex items-center gap-1'>
                    <FaWhatsapp size={20}/>
                    <p className='font-medium text-xs'>Whatsapp</p>
                </Link>

                <Link href={`https://twitter.com/intent/tweet?url=${shareLink}&text=${shareText}`} target='_blank' className='hover:bg-foreground/5 rounded transition p-2 flex items-center gap-1'>
                    <FaTwitter size={20}/>
                ...
                <Link href={`mailto:?subject=Check out this website!&amp;body=Check out this website`} target='_blank' className='hover:bg-foreground/5 rounded transition p-2 flex items-center gap-1'>
                    <Mail size={20}/>
