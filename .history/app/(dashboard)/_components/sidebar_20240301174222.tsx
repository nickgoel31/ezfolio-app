...
export const UpdatedSidebar = ({userId,username}:{userId:string,username:string}) => {
    ...
    return (
        <div className=''>
            {sidebarOpen && <div onClick={() => setSidebarOpen(!sidebarOpen)}  className='absolute w-full h-full bg-violet-950/10 backdrop-blur-lg z-[12]'></div>}

            
            <motion.div  onClick={() => setSidebarOpen(!sidebarOpen)} 
            className={cn('updated-sidebar cursor-pointer rounded-xl bg-gradient-to-t from-violet-950/10 to-background border p-5 font-semibold text-lg z-[14] fixed top-8 left-12 transition h-[70px] flex flex-col items-center justify-start', sidebarOpen && 'h-80')}>
