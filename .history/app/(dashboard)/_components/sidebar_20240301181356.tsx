...
export const UpdatedSidebar = ({userId,username}:{userId:string,username:string}) => {
    ...
    return (
        <div className='hidden lg:block w-full fixed'>
            {sidebarOpen && <div onClick={handleToggleSidebar}  
            className={cn('fixed w-full h-full bg-violet-950/10 backdrop-blur-lg z-[12] transition opacity-0', animation ? "opacity-100 animate-fade-in repeat-1" : "animate-fade-out repeat-1 opacity-0")}></div>}

            {sidebarOpen && (
                <div className={cn('z-[14] fixed right-12 top-10 cursor-pointer rounded-xl bg-gradient-to-t from-violet-950/10 to-background border p-2 px-4 transition opacity-0', animation ? "opacity-100 animate-fade-in repeat-1" : "animate-fade-out repeat-1 opacity-0")}>
                    <UserButton username={username}/>
                </div>
            )}
