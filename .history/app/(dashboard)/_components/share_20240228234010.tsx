...
import { Copy, CopyCheck, Mail, Share2, ShareIcon } from 'lucide-react'
...
const Share = ({username}:{username:string}) => {
  ...
  return (
    <div className={cn(isOpen ? "min-w-[26vw] p-3 px-4 flex flex-col gap-2 " : "p-2 px-3 min-w-0")}>
        {isOpen && <h1 className='font-semibold text-sm'>Share your page</h1>}
        ...
        {!isOpen && (
            <div onClick={() => setIsOpen(!isOpen)} className='w-full flex items-center gap-2'>
                <Share2 size={16}/>
