...
import UserButtonShort from './user-btn-short'
...
export const NewSidebar = ({userId,username,isAdmin}:{userId:string,username:string,isAdmin:boolean}) => {
    return(
            ...
            <div className='w-full h-full flex flex-col justify-between'>
                <div>
                    ...
                    <div className='navlinks flex flex-col gap-2 mb-6'>
                        {navigationItems.map((nav,index) => (
                            <div key={index} className={cn('w-full group')}>
                    ...
                    <div className='space-y-2'>
                        ...
                        <Link href={"https://forms.gle/WMbSaphG1TqQ1krM7"} target='_blank' className='flex items-center gap-3 w-full text-sm font-medium rounded hover:bg-foreground/5 p-2'>
                            <BugIcon size={18}/>
                            <p>Give Feedback</p>
                        </Link>
                ...
                <div>
                    <UserButton username={username} isAdmin={isAdmin}/>
                </div>
            </div>
        </div>
    )
...
export const SidebarV3 = ({userId,username,isAdmin}:{userId:string,username:string,isAdmin:boolean}) => {
    return(
        <div className='hidden lg:block fixed  w-[5vw] z-[10] border-r h-full   py-10 '>
            <div className='absolute w-full h-full z-[-1] left-0 top-0 overflow-hidden backdrop-blur-md '>
                <div className="dark:block fixed left-0 top-0 z-[1] bg-violet-600/50 rounded-full w-28 h-28 blur-3xl"></div>
                <div className="dark:block fixed right-0 bottom-0 z-[1] bg-blue-600/50 rounded-full w-28 h-28 blur-3xl"></div>
            </div>
            <div className='w-full h-full flex flex-col justify-between'>
                <div>
                    ...
                    <div className='navlinks flex flex-col gap-2 mb-6 items-center justify-center w-full'>
                        {navigationItems.map((nav,index) => (
                            <div key={index} className={cn('group ')}>
                                <NavItem  nav={nav} userId={userId} username={username}/>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className='space-y-2 w-full items-center justify-center flex flex-col'>

                        {/* <Link href={"mailto:harshgoel2004@gmail.com"} className='flex items-center gap-2 w-full text-sm font-medium rounded hover:bg-foreground/5 p-2'>
                            <CoffeeIcon size={17}/>
                            <p>Buy me a coffee</p>
                        </Link> */}
                        <Link href={`/connect`} className='relative group flex items-center gap-3 text-sm font-medium rounded hover:bg-foreground/5 p-2'>
                            <Users size={18}/>
                            <p className='absolute left-0 translate-x-[calc(5vw)] font-medium text-sm p-1 px-2 rounded-md bg-violet-950/10 border text-violet-200 group-hover:block hidden animate-fade-in duration-75'>EzConnect</p>
                        </Link>

                        <Link href={`/settings/${userId}`} className='relative group flex items-center gap-3 text-sm font-medium rounded hover:bg-foreground/5 p-2'>
                            <Settings2 size={18}/>
                            <p className='absolute left-0 translate-x-[calc(5vw)] font-medium text-sm p-1 px-2 rounded-md bg-violet-950/10 border text-violet-200 group-hover:block hidden animate-fade-in duration-75'>Settings</p>
                        </Link>

                        <Link href={"https://forms.gle/WMbSaphG1TqQ1krM7"} target='_blank' className='relative group flex items-center gap-3 text-sm font-medium rounded hover:bg-foreground/5 p-2'>
                            <BugIcon size={18}/>
                            <p className='absolute left-0 translate-x-[calc(5vw)] font-medium text-sm p-1 px-2 rounded-md bg-violet-950/10 border text-violet-200 group-hover:block hidden animate-fade-in duration-75'>Give Feedback</p>
                        </Link>
                ...
                <div className='w-full '>
                    {/* <UserButton username={username} isAdmin={isAdmin}/> */}
                    <UserButtonShort username={username} isAdmin={isAdmin}/>
                </div>
            </div>
        </div>
    )
