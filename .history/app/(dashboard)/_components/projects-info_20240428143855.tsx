...
const ProjectsInfo = ({projects,userPageId}:{projects:ProjectType[], userPageId:string}) => {
    ...
    const [loadingProject,setLoadingProject] = useState<ProjectStateType>()
    const [projectsState,setProjectsState] = useState<ProjectStateType[]>(projects)
    ...
    const handleProjectAdded = (newProject:ProjectStateType) => {
        setProjectsState([...projectsState, newProject]);
    };

    const handleProjectDelete = (id:string) => {
        setProjectsState(projectsState.filter(project => project.id !== id))
        DeleteProjectInDB(id)
    }
    

  return (
    <div id='project-info-dashboard' className='w-full flex flex-col space-y-4'>
        <div className='space-y-1 flex items-center justify-between w-full'>
            <h5 className='text-xl font-semibold'>Your Projects</h5>
            <AddProjectBtn userPageId={userPageId} onProjectAdded={handleProjectAdded} />
        </div>
