...
export const NavItem = ({nav, userId,username}:{nav:NavItemType, userId:string,username:string}) => {
  ...
  return (
    <>
      {!nav.preview ? (
        <Link href={`${nav.pathname}/${userId}${nav.href}`} className='rounded p-2 hover:bg-foreground/10 transition flex gap-4 items-center w-full justify-center lg:justify-start text-sm lg:text-base'>
          <Icon size={18}/>
          <p className='font-medium text-sm'>{nav.label}</p>
        </Link>
      ):(
        <Link href={`/${username}`} target='_blank' className='rounded p-2 hover:bg-foreground/10 transition flex gap-4 items-center w-full justify-center lg:justify-start text-sm lg:text-base'>
          <Icon size={18}/>
          <p className='font-medium text-sm'>{nav.label}</p>
