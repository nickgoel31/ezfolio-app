...
export const UpdatedSidebar = ({userId,username}:{userId:string,username:string}) => {
    ...
    return (
        <div className='hidden lg:block  w-full'>
            ...
            className={cn('updated-sidebar cursor-pointer rounded-xl bg-gradient-to-t from-violet-950/10 to-background border p-5 font-semibold text-lg z-[101] fixed top-8 left-12 transition h-[80px] flex flex-col items-center justify-start overflow-y-hidden', sidebarOpen && 'h-80')}>
                        ...
                        {(
                            <div className={cn('space-y-1 transition-opacity', animation ? "opacity-100 animate-fade-in repeat-1" : "animate-fade-out repeat-1 opacity-0")}>
                                ...
                                <div className='w-full relative'>
                                    ...
                                    <p className='absolute '>{nav.label}</p>
                                </div>
                                ))}
                            </div>
                        )}
