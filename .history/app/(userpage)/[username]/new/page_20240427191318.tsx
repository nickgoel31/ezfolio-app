...
const UserPage = async ({params}:{params:{username:string}}) => {
  ...
  return (
    <>
    <body className='w-full h-full relative flex flex-col items-center justify-start' style={{color: `${currentAdvancedPortfolioTheme.primaryFgColor}`, backgroundColor:`${currentAdvancedPortfolioTheme.primaryBgColor}`}}>
    ...
    <div className='sticky bottom-0 w-full bg-white z-[200] border-t border-black border-opacity-10 flex items-center justify-center shadow-md py-3'>
      ...
      <div className='w-full h-full flex '>
        <div className='flex-[3] w-full h-full relative overflow-hidden'>
          {/* BACKGROUND IMAGE */}
          <div className='absolute top-0 left-0 w-full h-full z-[-2]'>
            <Image src={"/new-port/1-img.jpeg"} alt='thumbnail' width={5000} height={5000} className='w-full h-full object-cover' />
            <span className='bg-black w-full h-full absolute top-0 left-0 opacity-50 '></span>
          </div>

          <div className='w-full h-full z-[4] flex flex-col items-center justify-center px-14 py-16'>
            <h1 className='text-5xl font-semibold text-white drop-shadow-lg' >{userPage.tagline}</h1>

            <Link href={"/"}>
              <Button variant={'outline'} size={"lg"} style={{color: `${currentAdvancedPortfolioTheme.primaryBgColor}`, backgroundColor:`${currentAdvancedPortfolioTheme.accentColor}`}}>Talk to me</Button>
            </Link>
          </div>
