...
import { TiptapAnswerAutomaticEzBuddyReadOnly, TiptapAnswerReadOnly, TiptapEzBuddyMessages } from '@/components/tiptap-read-only'
...
const EzBuddyChatBot = () => {
  ...
  for (let i = 0; i < Math.max(messages.length, htmlMessages.length); i++) {
    if (userMessages[i]) combinedMessages.push(messages[i]);
    if (modelMessages[i]) combinedMessages.push(htmlMessages[i]);
  }
  ...
  return (
    <div className='relative w-full  h-screen overflow-x-hidden mx-auto  px-2 pb-32'>
      ...
      <div className='fixed top-10 z-[100] bg-foreground/5 border rounded-full right-20 text-sm font-medium px-4 p-2'>
        <Link href={"/d/redirect"}>Back to Dashboard</Link>
      </div>
          ...
          <div className='h-full flex flex-col gap-4 items-center justify-between'>
            <div className='h-full flex flex-col gap-4 items-center'>
              ...
              <div className='text-center'>
                <h1 className='font-semibold text-6xl leading-normal font-poppins text-center'>Hi, I&apos;m <span className='ai-chatbot-gradient-text bg-gradient-to-tr from-teal-200 via-indigo-200 to-rose-200 bg-clip-text text-transparent'>EzBuddy</span></h1>
      ...
      <div className='space-y-4 max-w-screen-sm mx-auto py-10'>
            ...
            return (
              (
                <div key={index} className='relative'>
                  <div key={index} className={`p-3 px-4 rounded-lg ${msg.role === 'user' ? 'bg-foreground/5 text-foreground' : 'bg-violet-400/10 text-foreground'}`}>
                    {msg.role === "model" ? (
                      <Suspense fallback="loading...">
                        <TiptapEzBuddyMessages answer={msg.message}/>
                      </Suspense>
                    ):(
                      <div>{msg.message}</div>
                    )}
                  </div>
                    <div className={cn('absolute top-1/2 -translate-y-1/2', msg.role === 'user' ? 'right-[-20px]' : 'left-[-20px]')}>
                      <Avatar className=''>
                        <AvatarFallback>
        ...
        <div className="ai-response">
          {isLoading && (
            <div className={`p-4 px-4 rounded-lg bg-violet-400/10 text-foreground flex items-center justify-center`}>
              <ThreeDots
                visible={true}
                height="60"
                width="60"
