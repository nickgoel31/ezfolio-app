    ...
    <div className='w-full flex flex-col items-center space-y-7 relative'>
        <div className='absolute top-8 right-7'>
            <div onClick={toggleDetailedMode} className='flex items-center gap-3 font-medium text-sm'>
                ...
                <Switch checked={detailedMode} className='bg-violet-500'/>  
            </div>
        </div>
        ...
        <div className='flex flex-col lg:flex-row flex-wrap gap-8'>
            {pricingList.map((pricing) => (
                <motion.div key={pricing.id} className={cn('border rounded-xl w-[23rem] overflow-hidden  bg-gradient-to-t from-violet-800/10 via-transparent to-transparent transition h-[218px] ', detailedMode && 'h-[560px]')}>
                    <div className='rounded-xl shadow-md bg-gradient-to-tr from-violet-950/40 w-full p-5 px-6 space-y-3'>
                        ...
                        <div className='pt-2'>
                            <Button className='w-full bg-gradient-to-t shadow-sm from-violet-950 via-violet-950 to-violet-900 text-white'>{pricing.buttonLabel}</Button>
                        </div>
                    </div>
