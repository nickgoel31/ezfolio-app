...
const HelpCenter = () => {
  return (
    <div id='help' className='w-full flex flex-col items-center space-y-7 '>
        ...
        <div className='flex flex-col md:flex-row md:flex-wrap lg:flex lg:flex-wrap items-center justify-center max-w-screen-lg mx-auto  w-full gap-6 px-4 lg:px-0 bg-red-500'>
            {helpLinks.map((h,index) => {
                const Icon:LucideIcon | IconType = h.icon;
                return(
                    <div key={index} className='h-[260px] p-[1px] w-full max-w-[350px] relative rounded-xl  group'>
                        {/* BORDER */}
