...
const HelpCenter = () => {
  return (
    <div id='help' className='w-full flex flex-col items-center space-y-7'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 max-w-screen-lg w-full items-center gap-6'>
            {helpLinks.map((h,index) => {
                ...
                return(
                    <div className='h-[260px] p-[1px] relative rounded-xl overflow-hidden group'>
                        {/* BORDER */}
                        <div className='absolute top-0 left-0 w-full h-full dark:bg-[#131313] z-[-1]'>
                            <div className='group-hover:opacity-100 opacity-0 help-center-card-border absolute w-[200%] h-[200%] bg-gradient-to-tr from-transparent via-transparent to-foreground/20 top-[-50%] left-[-50%] transition-scroll-nav '>

                            </div>
                        </div>

                        {/* MAIN CONTENT */}
                        <Link href={h.url} className='rounded-xl shadow-xl p-6 px-8 space-y-4 flex flex-col h-full items-start justify-between   bg-gradient-to-tr from-[#f2f2f3] to-[#fefeff] dark:from-[#0d0d10] dark:to-[#09090b] cursor-pointer group'>
