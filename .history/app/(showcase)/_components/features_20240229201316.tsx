    ...
    <div className='w-full flex flex-col items-center space-y-7'>
        ...
        <div className='w-full max-w-screen-xl grid grid-cols-3 gap-4'>
            {featuresList.map((f,index) => (
                <div key={index} className='dark:features-card-dark border flex flex-col rounded-xl border-foreground/10 shadow-sm  p-[1.2px] h-64  relative group  w-full transition'>

                    <span className='features-border-div w-full h-full absolute top-0 left-0 bg-gradient-to-tr from-indigo-500 to-violet-500 z-[-1] group-hover:opacity-100 opacity-0 transition duration-300 scale-150'/>
