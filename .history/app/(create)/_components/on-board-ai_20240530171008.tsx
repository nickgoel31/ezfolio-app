  ...
  return (
    <div className='w-full h-full relative flex items-center justify-center max-w-screen-md mx-auto'>
        ...
        {currentSection === 4 && <div className='Message 1 leading-normal text-center font-poppins animate-fade-in'>
            ...
            <div>
                <Textarea className='mt-8 bg-foreground/[0.05] max-h-64 h-48' placeholder='Write about what you like, occupation, anything catchy...' defaultValue={}/>
            </div>

            <div className='w-full gap-4  mt-6 grid grid-cols-2 lg:grid-cols-4 items-start '>
                {bioSuggestions.map((sug) => (
                    <div key={sug.id} className='rounded-lg border border-muted bg-foreground/[0.05] p-2 w-full text-sm font-medium text-muted-foreground hover:text-foreground hover:border-foreground/80 cursor-pointer' onClick={}>
                        Template {sug.id}
                    </div>
                ))}
            </div>
