...
import { ColorScheme, PageFont, PageLayout } from '@prisma/client';
...
const OnBoardingAI = ({username,schemes,layouts,fonts}:{username:string,schemes:ColorScheme[], layouts:PageLayout[], fonts:PageFont[]}) => {

    const router = useRouter()

    const [currentSection,setCurrentSection] = useState<number>(10);
    ...
    const [currentThemeSelected,setCurrentThemeSelected] = useState<number>(1)
    const [currentFontSelected,setCurrentFontSelected] = useState<number>(1)
    ...
    const isNotValid = projectTitle === ""
      ...
      function onSubmit() {
        ...
        createUserPageInDB(values,projects,username,imageUrl,currentThemeSelected,currentFontSelected)
    ...
    <div className='w-full h-full relative flex items-center justify-center max-w-screen-md mx-auto'>
        ...
        {currentSection === 9 && (
            <div className='Message 1 leading-normal text-center font-poppins animate-fade-in'>
                ...
                <div className='flex gap-2 flex-wrap mt-6 items-center justify-center'>
        ...
        {currentSection === 10 && (
            <div className='Message 1 leading-normal font-poppins animate-fade-in w-full'>
                <h1 className='font-semibold text-2xl leading-normal text-center'>Set Appearance of your page<br /></h1>
                ...
                <div className='mt-6 w-full space-y-6'>
                    ...
                    <div className='flex flex-col gap-3 w-full'>
                        ...
                        <div className='flex flex-wrap gap-3 items-center'>
                            {schemes.map((scheme) => (
                                <div 
                                key={scheme.id} 
                                className={cn('border p-1 px-3 text-xs font-medium rounded-md cursor-pointer', {
                                  // Add classes conditionally if scheme id is matched
                                  'bg-foreground/[0.05]': scheme.id === currentThemeSelected,
                                })}
                                style={{
                                  // Apply inline styles conditionally if scheme id is matched
                                  backgroundColor: scheme.id === currentThemeSelected ? scheme.firstBgColor : undefined,
                                  color: scheme.id === currentThemeSelected ? scheme.fgColor : undefined,
                                  background: scheme.id === currentThemeSelected 
                                    ? `linear-gradient(45deg, ${scheme.firstBgColor} 0%, ${scheme.secondBgColor} 50%,${scheme.thirdBgColor} 100%)`
                                    : undefined,
                                }}
                                onClick={() => setCurrentThemeSelected(scheme.id)}
                              >
                                {scheme.name}
                              </div>
                            ))}
                    ...
                    <div className='flex flex-col gap-3 w-full'>
                        <h3 className='font-semibold text-lg'>Font</h3>
                        <div className='flex flex-wrap gap-3 items-center'>
                            {fonts.map((font) => (
                                <div 
                                key={font.id} 
                                className={cn('border p-1 px-3 text-xs font-medium rounded-md cursor-pointer', {
                                  // Add classes conditionally if scheme id is matched
                                  'bg-foreground/[0.1]': font.id === currentFontSelected,
                                })}
                                style={{
                                  // Apply inline styles conditionally if scheme id is matched
                                  fontFamily: font.family
                                }}
                                onClick={() => setCurrentFontSelected(font.id)}
                              >
                                {font.family}
                              </div>
                            ))}
