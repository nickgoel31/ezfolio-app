...
const QuestionPostContent = async ({question}:{question:QuestionPost}) => {
    const currentUser = useCurrentUser()
    const isPostCreator = question.userId === currentUser?.id || false

    const htmlDesc = await marked(question.description)
  ...
  return (
    <div className='space-y-6  '>
        ...
        <TiptapQuestionReadOnly description={htmlDesc} isPostCreator={isPostCreator} questionPostId={question.id}/>
    </div>
  )
}
