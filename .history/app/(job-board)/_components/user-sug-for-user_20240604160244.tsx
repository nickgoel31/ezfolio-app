            ...
            <CarouselContent className='space-x-6'>
                {suggestedUsers.map((user) => (
                    <CarouselItem key={user.id} className="2xl:basis-1/6 xl:basis-1/5 lg:basis-1/3 md:basis-1/2 basis-1/1">
                        <Link href={`/${user.username}`} className=' w-56 bg-foreground/[0.02] relative overflow-hidden rounded-lg border border-foreground/5 p-4 py-6 gap-3 flex flex-col items-center group over'>
