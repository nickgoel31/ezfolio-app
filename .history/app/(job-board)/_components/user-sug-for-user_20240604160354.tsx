                ...
                {suggestedUsers.map((user) => (
                    <CarouselItem key={user.id} className="2xl:basis-1/6 xl:basis-1/5 lg:basis-1/3 md:basis-1/2 basis-1/1">
                        <Link href={`/${user.username}`} className=' w-56 bg-foreground/[0.02] relative overflow-hidden rounded-lg border border-foreground/5 p-4 py-6 gap-3 flex flex-col items-center group'>
                            <div className='group-hover:opacity-100 absolute top-0 left-0 w-full h-full bg-background/40 backdrop-blur-lg z-[3] flex items-center justify-center font-medium text-lg'>
                                Visit
                            </div>
