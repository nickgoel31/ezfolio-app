        ...
        <div className='flex gap-4'>
          <div className='grid grid-cols-1 w-full gap-4 flex-[3]'>
            {questionPosts.map((q)=> {
              ...
              const htmlText = q.des
              const tempDiv = document.createElement('div');

              // Set the HTML content of the div with your HTML text
              tempDiv.innerHTML = htmlText;
              return (
              <Link href={`/connect/questions/${q.id}`} key={q.id} className='bg-foreground/5 rounded-lg p-3'>
                <h3 className='text-lg font-semibold'>{q.title}</h3>
                <p className='text-muted-foreground text-sm font-medium line-clamp-2'>{q.description}</p>
              </Link>
          )})}
          </div>
