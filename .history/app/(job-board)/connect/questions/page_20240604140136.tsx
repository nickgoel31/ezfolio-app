      ...
      <div className='space-y-14 lg:px-24'>
        ...
        <div className='flex gap-4'>
          ...
          <div className='flex-[1] w-full'>
            <Suspense>
              <div className='bg-foreground/5 rounded-lg p-3 px-4 font-medium'>
                Trending Questions
                <div className='pt-4 space-y-3'>
                  {trendingQuestions.map(async (ques) => {
                    const likes = await getLikesForQuestionPost(ques.id) 
                  return(
                    <div key={ques.id}>
                      <Link className='text-sm font-normal  text-muted-foreground flex items-center justify-between w-full' href={`/connect/questions/${ques.id}`}>
                        <p className='underline underline-offset-2'>{ques.title}</p>
                        <div className='flex items-center gap-1 text-xs no-underline'>
                          <ThumbsUp size={14}/>
                          <p className='no-underline'>{likes.length}</p>
                        </div>
                      </Link>
                    </div>
                  )})}
