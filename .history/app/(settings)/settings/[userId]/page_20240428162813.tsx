...
const SettingsPage = async ({params}:{params:{userId:string}}) => {
    ...
    const userPage = await getUserpageByUserId(userId)
    if(!userPage) redirect("/s")
  return (
        <div className='flex flex-row items-start w-full gap-10 pt-4 pb-16'>
            <div className='flex-[2] sticky top-0'>
                <SettingsSidebar />
            </div>
            <div className='flex-[8] rounded-xl bg-foreground/5 sm:px-8 sm:shadow space-y-8'>
                <GeneralSettings user={user}/>
                <UserpageSettings userpage={userPage}/>
            </div>
        </div>
