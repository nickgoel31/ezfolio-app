...
import { enableTwoFactorAuthenticationInDB } from '@/actions/auth/2fa'
...
const GeneralSettings = ({user}:{user:User}) => {
    ...
    function handle2FAEnable () {
        enableTwoFactorAuthenticationInDB(user.id)
        .then(d => {
            if()
        })
    }
       ...
       <section className='space-y-8'>
            ...
            <div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button onClick={handle2FAEnable} disabled={user.twoFactorAuthentication} className="inline-flex text-sm font-semibold text-blue-600 disabled:text-muted-foreground underline decoration-2">Enable</button>
                </div>
            </div>
       </section>
    </section>
