  ...
  return (
      <ul>
        {settingsNavbarItems.map((item,index) => (
            <Link href={} key={index} className="mt-5 cursor-pointer border-l-2 border-transparent px-2 py-2 font-semibold transition hover:border-l-blue-700 hover:text-blue-700">{item.label}</Link>
        ))}
