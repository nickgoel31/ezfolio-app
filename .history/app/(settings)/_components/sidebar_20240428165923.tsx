  ...
  return (
      <ul className='flex flex-col'>
        {settingsNavbarItems.map((item,index) => (
            <Link href={item.path} key={index} className={cn("mt-5 cursor-pointer border-l-2 border-transparent px-2 py-2 font-semibold transition hover:border-l-blue-500 hover:text-blue-500", hash === item. ? "border-l-blue-500 text-blue-500" : "")}>{item.label}</Link>
        ))}
