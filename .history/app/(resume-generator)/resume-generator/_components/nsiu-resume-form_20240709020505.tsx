...
const ResumeGeneratorFormNSIU = () => {
    ...
    function getLocalStorageValues(){
        if (typeof window !== "undefined"){
            ...
            const awards = localStorage.getItem("awards")
            const certifications = localStorage.getItem("certifications")

            if (name && email && bio && experience && education && skills && projects && awards && certifications){
                form.setValue("name", name)
                form.setValue("email", email)
                form.setValue("bio", bio)
                setExperiences(JSON.parse(experience))
                setEducations(JSON.parse(education))
                setSkills(JSON.parse(skills))
                setProjects(JSON.parse(projects))
                setAwards(JSON.parse(awards))
                setCertifications(JSON.parse(certifications))
            }
        }
    }
  return (
    <>
        ...
        <div className='flex flex-col items-start gap-8 pt-5 pb-20 px-6'>
        {/* PROFILE PIC */}
