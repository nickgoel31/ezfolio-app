...
const VerifyEmailPage: NextPage<VerifyEmailProps> = ({ emailVerificationToken }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!emailVerificationTokenFromDB) {
        return; // No need to set errors if the token is invalid
      }

      // Client-side validation can be added here for additional security

      try {
        const response = await updateEmailVerified(emailVerificationTokenFromDB.userEmail, emailVerificationTokenFromDB.token);
