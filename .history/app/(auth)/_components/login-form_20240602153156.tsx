...
export default function LoginForm() {
  ...
  const handle2FAVerification = () => {
    ...
    verify2FACodeAndLogin(OTP, twoFATokenId)
  }

  return (
    <Form {...form}>
      {error && (
          <div className="border border-red-500 bg-red-500/10 p-4 rounded max-w-[400px] text-sm font-medium text-red-500 flex items-center gap-3 absolute top-5 left-1/2 -translate-x-1/2 z[]">
        ...
        {!twoFADialog && (
          <div className="w-full h-full absolute top-0 left-0 z-[60] bg-background/5 backdrop-blur-lg flex items-center justify-center">
            <div className="p-6 w-[90%] max-w-[500px] items-center justify-center flex border bg-background rounded-lg flex-col">
              ...
              <div className="pt-6">
                <Button type="button" disabled={OTP.length < 6} onClick={handle2FAVerification} className="w-full">
