...
const AITest = () => {
  ...
  const [messages,setMessages] = useState<HistoryMessage[]>([{role:"user", message:"Hello how are you?"},{role:"model", message:"I'm also doing fine!"}])
    ...
    <div className='max-w-screen-sm mx-auto py-10 px-2'>
      ...
      <div className='space-y-2'>
        {messages.map((msg,index) => (
          <div key={index} className={`p-2 rounded-lg ${msg.role === 'user' ? 'bg-foreground/5 text-foreground' : 'bg-violet-500/5 text-foreground'}`}>{msg.message}</div>
        ))}
      </div>
