...
import { BookOpen, Code2, MessagesSquareIcon, Paintbrush } from 'lucide-react'
...
const AITest = () => {
  ...
  return (
    <div className='relative w-full  h-screen overflow-x-hidden mx-auto  px-2 pb-32'>
      ...
      {messages.length > 0 && (
        <div className='sticky top-6 w-full max-w-screen-sm mx-auto py-8 z-[87]'>
          <div className='absolute top-0 left-0 p-4 border w-full rounded-lg bg-neutral-900 flex items-center justify-between'>
            ...
            <div>
              <button className='flex gap-2 font-medium items-center text-sm border'  onClick={() => setMessages([])}>
                <Paintbrush size={20}/>
                Clear chat
              </button>
            </div>
      ...
      <div className='fixed w-full max-w-scree bottom-10 left-1/2 -translate-x-1/2 p-2 rounded-lg bg-neutral-100 dark:bg-neutral-900 z-[10]'>
        ...
        <p className='text-center font-medium text-xs text-muted-foreground mt-2'>
          EzBuddy is currently in beta phase.
        </p>
      </div>
    </div>
  )
