...
const AITest = () => {
  ...
  const [messages, setMessages] = useLocalStorage<HistoryMessage[]>("messages", [])

  useEffect(() => {
    // Ensure messages are set from local storage on initial render
  ...
  return (
    <div className='relative w-full  h-screen overflow-x-hidden mx-auto  px-2 pb-32'>
      ...
      {messages.length === 0 && (
        <div className='w-full h-full flex flex-col items-center justify-between max-w-screen-sm mx-auto space-y-6'>
      ...
      {messages.length > 0 && (
        <div className='sticky top-6 w-full max-w-screen-sm mx-auto py-8 '>
          <div className='absolute top-0 left-0 p-4 border w-full rounded-lg bg-neutral-900 flex items-center justify-between'>
            <div>
              <span className='bg-gradient-to-tr from-teal-200 via-indigo-200 to-rose-200 bg-clip-text text-transparent font-medium text-lg'>EzBuddy</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className='fixed bottom-10 left-1/2 -translate-x-1/2 p-2 rounded-lg bg-foreground/5 z-[10]'>
        <form onSubmit={handleClick} className='flex gap-2 items-center'>
          <Textarea ref={inputRef} className='w-96 h-12 max-h-64 min-h-12' placeholder='Write something here' onChange={(e) => setText(e.target.value)} value={text} />
        ...
        <p className='text-center font-medium text-xs text-muted-foreground mt-2'>
          Currently EzBuddy cannot save chat history.
        </p>
      </div>
    </div>
  )
