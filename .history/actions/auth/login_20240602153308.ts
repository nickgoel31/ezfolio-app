...
export const verify2FACodeAndLogin = async (pin:string , tokenId:string) => {
    ...
    if(!twoFaToken) return {error:"Token Expired! Login again."}

    if(twoFaToken.pin !== pin) return {error:"Incorrect Pin! Input Again."}

    const existingUser = await getUserByEmail(twoFaToken.userEmail)
    if(!existingUser) return {error: "No user found! Please sign up."}
