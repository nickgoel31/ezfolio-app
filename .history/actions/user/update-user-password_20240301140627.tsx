...
export const updateEmailVerified = async (email:string,token:string) => {
    ...
    const currToken = await getEmailVerificationTokenByToken(token)
    if(!currToken) return {error: "Invalid Token"}

    const isValidToken = checkEmailVerificationToken(currToken)
    if(!isValidToken) return {error: "Token expired, Please request a new one"}
