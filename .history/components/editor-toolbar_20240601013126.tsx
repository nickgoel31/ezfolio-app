...
import { AlignCenter, AlignJustify, AlignLeft, AlignRight, Bold, Code, Heading1, Heading2, Heading3, Heading4, Highlighter, Italic, Link, List, ListOrdered, Quote, Strikethrough, Underline } from 'lucide-react'
  ...
  return (
    <ContextMenuContent>
        ...
        <div className='w-full rounded-md p-3 grid grid-cols-4 items-center gap-2'>
            ...
            {/* LINK */}
            <Toggle size={"sm"} className='border' pressed={editor.isActive("link")} onPressedChange={() => editor.chain().focus().toggleLink({ href: 'https://example.com' }).run()}>
                <Link size={20}/>
            </Toggle>

            {/* underline */}
            <Toggle size={"sm"} className='border' pressed={editor.isActive("underline")} onPressedChange={() => editor.chain().focus().toggleUnderline().run()}>
                <Underline size={20}/>
            </Toggle>

            {/* STRIKETHROUGH */}
            <Toggle size={"sm"} className='border' pressed={editor.isActive("strike")} onPressedChange={() => editor.chain().focus().toggleStrike().run()}>
