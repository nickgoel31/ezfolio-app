...
const EditorToolbar = ({editor}:{editor:Editor | null}) => {
  ...
  return (
    <div className='w-full rounded-md border border-input bg-background p-3 flex items-center gap-2'>
        {/* HEADING */}
        <DropdownMenu>
        <DropdownMenuTrigger className='font-bold p-1 px-3 rounded-md hover:bg-foreground/10 text-sm border '>Heading</DropdownMenuTrigger>
        <DropdownMenuContent>
            <DropdownMenuItem>
        ...
        {/* PARAGRAPH */}
        <Toggle size={"sm"} className='border ' pressed={editor.isActive("heading", {level:4})} onPressedChange={() => editor.chain().focus().toggleHeading({level:4}).run()}>
            <FaParagraph size={20}/>
        </Toggle>
