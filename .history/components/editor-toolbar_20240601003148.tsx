          ...
          <ContextMenuSubContent className="w-48">
            <ContextMenuItem>
                <Toggle size={"sm"} className='w-full' pressed={editor.isActive("heading", {level:1})} onPressedChange={() => editor.chain().focus().toggleHeading({level:1}).run()}>
                        <Heading1 size={20}/>
                <ContextMenuShortcut>⌘+AltS</ContextMenuShortcut>
                </Toggle>
            </ContextMenuItem>
            ...
            {/* HEADING */}
            <DropdownMenu>
            <DropdownMenuTrigger className='font-bold p-1 px-3 h-full rounded-md hover:bg-foreground/10 text-sm border '></DropdownMenuTrigger>
            <DropdownMenuContent>
                <DropdownMenuItem>
