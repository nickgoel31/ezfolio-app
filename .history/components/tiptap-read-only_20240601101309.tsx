...
export const TiptapAnswerReadOnly = ({description,isPostCreator,answerId}:{description:string,isPostCreator:boolean,answerId:string}) => {
  const [editState,setEditState] = useState(false)
  const [ansState, setAnsState] = useState(description)
    const editor = useEditor({
      ...
      function onSubmit () {
        ...
        updateAnswerInDB(savedAnsDesc,answerId,userId)
        .then(d => {
          if(d.success) {
            toast({
              title: "Question Updated",
