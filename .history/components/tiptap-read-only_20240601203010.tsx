...
export const TiptapAnswerAutomaticEzBuddyReadOnly = ({description,isPostCreator,answerId,answerUserId}:{description:string,isPostCreator:boolean,answerId:string,answerUserId:string}) => {
  const [editState,setEditState] = useState(false)
  const [ansState, setAnsState] = useState(description)
  const { toast } = useToast()
    const editor = useEditor({
