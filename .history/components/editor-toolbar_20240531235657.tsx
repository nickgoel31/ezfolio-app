...
import { Bold, Code, Heading1, Heading2, Heading3, Heading4, Italic } from 'lucide-react'
...
const EditorToolbar = ({editor}:{editor:Editor | null}) => {
  ...
  return (
    <div className='w-full rounded-md border border-input bg-background p-3 flex items-center gap-2'>
        ...
        {/* Code */}
        <Toggle size={"sm"} className='border' pressed={editor.isActive("blockquote")} onPressedChange={() => editor.chain().focus().toggleCode().run()}>
            <Code size={20}/>
        </Toggle>
