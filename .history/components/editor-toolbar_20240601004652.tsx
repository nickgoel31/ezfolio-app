...
import { AlignCenter, AlignJustify, AlignLeft, AlignRight, Bold, Code, Heading1, Heading2, Heading3, Heading4, Italic, ListOrdered, Quote } from 'lucide-react'
  ...
  return (
    <ContextMenuContent>
        ...
        <ContextMenuSub>
          ...
          <ContextMenuSubContent className="w-48">
            <ContextMenuItem>
                <Toggle size={"sm"} className='w-full' pressed={editor.isActive("rightAlign")} onPressedChange={() => editor.chain().focus().setTextAlign('left').run()}>
                    <AlignLeft size={20}/>
                    <ContextMenuShortcut>⌘Alt1</ContextMenuShortcut>
                </Toggle>
            </ContextMenuItem>
            <ContextMenuItem>
                <Toggle size={"sm"} className='w-full' pressed={editor.isActive("heading", {level:2})} onPressedChange={() => editor.chain().focus().setTextAlign('center').run()}>
                    <AlignCenter size={20}/>
                    <ContextMenuShortcut>⌘Alt2</ContextMenuShortcut>
                </Toggle>
            </ContextMenuItem>
            <ContextMenuItem>
                <Toggle size={"sm"} className='w-full' pressed={editor.isActive("heading", {level:3})} onPressedChange={() => editor.chain().focus().setTextAlign('right').run()}>
                    <AlignRight size={20}/>
                    <ContextMenuShortcut>⌘Alt3</ContextMenuShortcut>
                </Toggle>
            </ContextMenuItem>
            <ContextMenuItem>
                <Toggle size={"sm"} className='w-full' pressed={editor.isActive("heading", {level:4})} onPressedChange={() => editor.chain().focus().setTextAlign('justify').run()}>
