...
const UserpageAdvancedPortfolioPageModernTheme = ({userPage,title,bio,id,projects,experiences,skills,colorScheme,currentFont, pageLayout, hasSocialLinks, fontWeight, vipTag,uniqueTag, currentAdvancedPortfolioTheme}:Props) => {
  ...
  return (
    ...
    <div className='w-full min-h-full relative flex flex-col items-center justify-start' style={{fontFamily: `${currentFont.family}`,backgroundColor:`${currentAdvancedPortfolioTheme.primaryBgColor}`}}>
          ...
          <div className='flex flex-col gap-8'>
            {experiences.map((exp) => (
              <div key={exp.id} className='flex items-center gap-3'>
                <div className='rounded-full p-3 ' style={{color: `${currentAdvancedPortfolioThemeC.primaryFgColor}`, backgroundColor:`${currentAdvancedPortfolioThemeC.primaryBgColor}`}}>
                  <Building2 />
                </div>
      ...
      <div id='projects' className='  w-full flex flex-col items-center justify-center px-32 pt-16 mx-auto space-x-14 pb-16 space-y-8' style={{color: `${currentAdvancedPortfolioThemeC.secondaryFgColor}`, backgroundColor:`${currentAdvancedPortfolioThemeC.secondaryBgColor}`}}>
          <h1 className='text-3xl font-semibold'>
          Projects
          </h1>      
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6'>
              {projects.map((project,index) => (
                <div key={index} className=' rounded-xl p-6 space-y-4' style={{color: `${currentAdvancedPortfolioThemeC.primaryFgColor}`, backgroundColor:`${currentAdvancedPortfolioThemeC.primaryBgColor}`}}>
                ...
                <Button variant={'outline'} className=' mt-2' style={{color: `${currentAdvancedPortfolioThemeC.primaryBgColor}`, backgroundColor:`${currentAdvancedPortfolioThemeC.accentColor}`}}>Visit</Button>
              </div>
              ))}
          </div> 
      </div>

      <div className='py-14 w-full flex flex-col items-center justify-center px-32 mx-auto space-x-10 ' style={{color: `${currentAdvancedPortfolioThemeC.primaryFgColor}`, backgroundColor:`${currentAdvancedPortfolioThemeC.primaryBgColor}`}}> 
        <div className='flex flex-col gap-2 items-center justify-center'>
          ...
          <div className='pt-2'>
            <Button variant={'outline'} style={{color: `${currentAdvancedPortfolioThemeC.primaryBgColor}`, backgroundColor:`${currentAdvancedPortfolioThemeC.accentColor}`}}>Talk to me</Button>
          </div>
        </div>
    ...
    <div className='sticky bottom-0 w-full  z-[200] flex items-center justify-center shadow-md backdrop-blur-lg py-3 ' style={{color: `${currentAdvancedPortfolioThemeC.primaryFgColor}`, backgroundColor:`${currentAdvancedPortfolioThemeC.primaryBgColor}90`, borderTop: `1px ${currentAdvancedPortfolioThemeC.primaryFgColor}20 solid`}}>
      <Link href={"https://ezfolio.social/s"} target='_blank' className='text-center font-medium' style={{color:`${currentAdvancedPortfolioThemeC.primaryFgColor}`}}>
