...
const LikeBtn = ({currentUser,questionId,isLikedPost}:{currentUser:User, questionId:string,isLikedPost:boolean}) => {
    const [liked,setLiked] = useState<boolean>(isLikedPost)
  ...
  return (
        <div onClick={handleLikePost} className={cn('fixed bottom-10 right-10 font-medium z-[80] p-3 rounded-full bg-foreground text-background cursor-pointer', liked && 'bg-violet-500 text-foreground')}>
            {liked ? (<FaThumbsUp size={30}/>):(<ThumbsUp size={30}/>)}
        </div>
  )
}
